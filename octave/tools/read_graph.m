...
while true
  ...
  if (strcmp(tokens(1), 'VERTEX_SE2') ~= 0)
    ...
    graph.idLookup = setfield(graph.idLookup, {id}, 'offset', length(graph.x));
    graph.idLookup = setfield(graph.idLookup, {id}, 'dimension', length(values));
  ...
  elseif (strcmp(tokens(1), 'EDGE_SE2') ~= 0)
    ...
    graph.edges = [graph.edges; struct('type', 'P','from', fromId,'to', toId,'measurement', measurement,'information', information)];
  elseif (strcmp(tokens(1), 'EDGE_SE2_XY') ~= 0)
    ...
    graph.edges = [graph.edges; struct('type', 'L','from', fromId,'to', toId,'measurement', measurement,'information', information)];
  end
...
for eid = 1:length(graph.edges)
  graph.edges(eid).fromIdx = getfield(graph.idLookup, {graph.edges(eid).from}).offset + 1;
  graph.edges(eid).toIdx = getfield(graph.idLookup, {graph.edges(eid).to}).offset + 1;
end
